...
def create_data_model():
    ...
    'demands': [0, 1, 1, 3, 6, 3, 6, 8, 8, 1, 2, 1, 2, 6, 6, 8, 8], 'vehicle_capacities': [12, 14, 16, 18],
        'num_vehicles': 4, 'depot': 0,"route": [],"solution": {}}
    return data
...
def plot_solution(data):
    ...
    vehicle_color_map = ['blue', 'orange', 'yellow', 'green']
    ...
    for vehicle_id, vehicle_route in enumerate(data['route']):
        ...
        for idx, node in enumerate(vehicle_route):
            ...
            node_color_dict[node] = vehicle_color_map[vehicle_id]
            edges_loaded_labels[(previous_node, node)] = data['demands'][previous_node]
            edges_list.append((previous_node, node))
            previous_node = node

        # back to depot
        edges_list.append((previous_node, 0))
        edges_loaded_labels[(previous_node, 0)] = data['demands'][previous_node]
    ...
    for vehicle_id, edges in vehicle_edges_dict.items():
        ...
        nx.draw_networkx_edges(di_graph, pos_nodes, edgelist=edges, width=3, alpha=0.9, arrowstyle='simple'
                        ,arrowsize=20, node_size=1500,
                         arrows=True, edge_color=vehicle_color_map[vehicle_id])
    ...
    print('edges_loaded_labels',edges_loaded_labels)
